...
# Residence area type filter (dropdown)
selected_area_type = st.sidebar.selectbox(
    "Select Residence Area Type",
    options=df['Dim1'].unique(),
    index=0  # Default to first option
)

# ========================= Apply Filters ==========================
# Filter data based on year, selected regions, and residence area type
filtered_df = df[
    (df['Year'] == selected_year) &
    (df['ParentLocation'].isin(selected_regions)) &
    (df['Dim1'] == selected_area_type)
]
...
with tab1:
    ...
    st.subheader(f"Showing data for {selected_area_type} areas")
