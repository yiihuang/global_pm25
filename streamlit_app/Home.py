...
# ======================== Page Configuration ========================
st.title("Global PM2.5 Air Quality Analysis")  # Page title
st.markdown("""
Explore PM2.5 (µg/m³) measurements across different regions and locations. 
Data source: [WHO Air Quality Database 2022](https://www.who.int/data/gho/data/themes/air-pollution/who-air-quality-database/2022)
""")  # Short description with data source link
...
selected_dim1 = st.sidebar.selectbox(
...
with tab1:
    # Global PM2.5 Overview Map
    st.subheader(f"Global PM2.5 Overview for {selected_dim1} Areas")
    st.plotly_chart(
        create_choropleth(filtered_df, selected_years[1]),  # Use the end year of the range
...
with tab2:
    st.subheader(f"Interactive PM2.5 Statistics for {selected_dim1} Areas")
...
# ======================== TAB 3: Raw Data ==============================
with tab3:
    st.subheader(f"Filtered Raw Data for {selected_dim1} Areas")
...
st.subheader(f"Key Metrics for {selected_dim1} Areas")
